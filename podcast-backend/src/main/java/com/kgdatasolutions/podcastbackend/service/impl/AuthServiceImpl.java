...
import com.kgdatasolutions.podcastbackend.Model.*;
        ...
        try {
            ...
            if (user.getEpisodes() != null && !user.getEpisodes().isEmpty()) {
                for (Episode episode : user.getEpisodes()) {
                    ...
                    gcsService.deleteFileFromGCS(episode.getAudioUrl());
                }
            }

            // Check if the user has a profile URL
            if (user.getProfileUrl() != null) {
                gcsService.deleteFileFromGCS(user.getProfileUrl());
            }

            // Check if the user has a banner URL
            if (user.getBannerUrl() != null) {
                gcsService.deleteFileFromGCS(user.getBannerUrl());
            }
